...
\begin{tikzpicture}[unipd/diagram,node distance=16mm]
 ...
 \draw[unipd/graph directed] (a)--node[unipd/edge label,above]{$2$}(b);
 \draw[unipd/graph directed] (b)--node[unipd/edge label,above]{$5$}(c);
 \draw[unipd/graph optional,bend right=28] (a) to node[unipd/edge label,below]{$8$}(c);
\end{tikzpicture}
...
\begin{tikzpicture}[unipd/diagram,node distance=20mm]
 ...
 \path[unipd/automaton transition] (q0) edge node[unipd/transition label,above]{$1$}(q1)
   (q0) edge[unipd/automaton loop,loop above] node[unipd/transition label]{$0$}(q0)
   (q1) edge[unipd/automaton loop,loop above] node[unipd/transition label]{$0,1$}(q1);
\end{tikzpicture}
