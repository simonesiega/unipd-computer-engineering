...
\makeatletter
\newcommand{\printintegrationlists}{%
  \clearpage
  \chapter*{Elenchi del documento}%
  \addcontentsline{toc}{chapter}{Elenchi del documento}%
  \section*{\listfigurename}\@starttoc{lof}\medskip
  \section*{\listtablename}\@starttoc{lot}\medskip
  \section*{\listalgorithmcfname}\@starttoc{loa}\medskip
  \section*{\lstlistlistingname}\@starttoc{lol}%
}
\makeatother
...
Se il contatore parte da $n=0$ ed è modificato soltanto mentre il mutex è acquisito, ogni esecuzione conserva l'invariante
\[
  0 \leq n \leq N.
\]
\end{theorem}
...
Sulla macchina di laboratorio la sezione critica non contesa richiede \qty{1.5}{\micro\second}; tre misure del risveglio di un thread sono \numlist{8.2;8.7;9.1}\unit{\micro\second}. Conviene quindi preparare i dati fuori dal mutex e limitare la regione protetta all'aggiornamento degli indici e del contatore.

\begin{theorem}\label{thm:fifo}
Se l'inserimento scrive nella posizione $t$ prima di incrementarla e la rimozione legge la posizione $h$ prima di avanzare, gli elementi sono restituiti in ordine FIFO.
\end{theorem}
\begin{proof}
Entrambi gli indici avanzano di una posizione modulo $N$. Una cella non può essere riutilizzata prima della sua lettura, perché il \cref{thm:bounds} impedisce al produttore di superare il consumatore. Le rimozioni visitano pertanto le celle nello stesso ordine in cui sono state riempite.
\end{proof}
...
\chapter{Architettura e misure sperimentali}
...
Il benchmark elabora un milione di messaggi dopo una fase di riscaldamento. Ogni risultato è la mediana di sette esecuzioni e il throughput conta le rimozioni completate, non i tentativi di inserimento. La \cref{tab:throughput} evidenzia il compromesso tra capacità utile e costo della sincronizzazione.

\begin{table}[H]
  \centering
  \caption{Prestazioni al variare della capacità del buffer}\label{tab:throughput}
  \begin{tabular}{rrr}
    \toprule
...
\begin{code}[language=Python,caption={Inserimento con controllo esplicito},label={lst:insert}]
...
\begin{code}[language=Python,caption={Rimozione con controllo esplicito},label={lst:remove}]
def dequeue(queue):
    with queue.mutex:
        if queue.count == 0:
